package table4K.data;

import javafx.scene.image.Image;

import static table4K.data.RootPaneBackgroundData.createImage;



public class IconData {

    public static Image getBookIconImage() {
        return createImage("icon/book_icon.png");
    }

    public static Image getFilmIcon1Image() {
        return createImage("icon/film1_icon.png");
    }
    public static Image getFilmIcon2Image() {
        return createImage("icon/film2_icon.png");
    }

    public static Image getMailIconImage() {
        return createImage("icon/mail_icon.png");
    }

    public static Image getMapIconImage() {
        return createImage("icon/map_icon.png");
    }

    public static Image getMedalIconImage() {
        return createImage("icon/medal_icon.png");
    }

    public static Image getPortfolioIconImage() {
        return createImage("icon/portfolio_icon.png");
    }

    public static Image getQuizIconImage() {
        return createImage("icon/quiz_icon.png");
    }

}
